...
export class CrimeController {
  ...
  public async listaArmasDoCrime(req: Request, res: Response) {
    try {
      const { id } = req.params;

      const result = await crimeService.listArmasDoCrime(id);
      return res.status(200).send({ message: "Armas usadas nesse crime:", data: result });
    } catch (error) {
      return res.status(500).send(error);
    }
  }
